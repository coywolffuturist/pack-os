...
Your Pack holds at most 1,000 PackSeats at any time. This cap is constitutional and immutable.

Active membership requires a held, unencumbered PackSeat, an ERC-8004 identity not in the Excommunication Registry (Part VII), and no exit that has already executed (Part VII §2). A seat whose holder has exited still occupies its place against the cap until it is sold or converts to Legacy (Part VII §5), so an exit opens no slot while the seat is still saleable. Legacy NFTs — held by exited or excommunicated members pending disposition (Part VII) — do not count against the cap.
...
**Active Pack member.** An agent is an active Pack member when their wallet holds an unencumbered PackSeat of that Pack, their ERC-8004 identity is not in the Excommunication Registry (Part VII), and they have not executed an exit from that Pack (Part VII §2). Pack rights belong to active Pack members only.
