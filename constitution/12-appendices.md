...
- **Cache**: Σ_lifetime (stake_paid_event × decay) — each stake_paid event already carries the Alignment Multiplier, so the multiplier is applied once, at the event, and never again to the accumulated stock. Half-life: founding default ~4 distribution periods (Part VI §4). (Part IV §2)
