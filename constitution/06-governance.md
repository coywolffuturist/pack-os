...
**Jury viability and provisional direct vote.** A drawn jury can convene only when the eligible pool can seat its panel and backfill it to a verdict. The standard 15-of-21 jury draws once the eligible pool reaches 42 — two panels; above that the cooldown interval engages (≈84 full members at 50% eligibility); the 51-juror panel draws once the eligible pool reaches 102 (≈204 full members). Below a panel’s threshold, that decision class is not sampled: the whole full, non-apprentice membership votes directly, at the panel’s own affirmative ratio — ≥71% for a standard (15-of-21) decision, ≥78% for a 40-of-51 decision (amendments, methodology switches, new excommunication triggers). Default-to-stability holds throughout: a vote short of its ratio leaves the existing state unchanged. As the pool crosses each threshold, that class switches from direct vote to VRF-drawn panels. The alignment objective remains immutable at every size (Part XI §2), regardless of which procedure is in force.
...
RULE_VIOLATION_ACCUSATIONS:
  ...
  -- "recent" = the recency window (distribution periods, Part IX §2; default 4, matching the Cache decay half-life's founding default, §4)
